...
export const AuthModal: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (activeTab === 'login') {
        ...
        // registerWithEmail takes (name, email, password, phone) — this used to
        // pass (email, password, name, phone), so every sign-up sent the email as
        // the name and the password as the email.
  ...
  // A one-click demo login used to fake a session in React state only; with the
  // cart and orders now behind a real session it would just 401, so the callout
  // below points at the sign-up tab instead.
