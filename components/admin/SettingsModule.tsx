...
export const SettingsModule: React.FC = () => {
  ...
  return <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
    ...
    <div className="space-y-6 p-5 md:p-7">
      ...
      {tab === 'general' && <section className="space-y-6">
        <div>
          <h3 className="section-title font-bold text-slate-800 text-sm">Business details</h3>
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            <Field label="Store name" value={profile.storeName} onChange={(v) => set('storeName', v)} />
            <Field label="Registered legal name" value={profile.legalName} onChange={(v) => set('legalName', v)} />
            <Field label="9-digit PAN / VAT number" value={profile.panVatNumber} onChange={(v) => set('panVatNumber', v)} placeholder="302910482" />
            <Field label="Support phone number" value={profile.contactPhone} onChange={(v) => set('contactPhone', v)} />
            <Field label="Public support email" value={profile.contactEmail} onChange={(v) => set('contactEmail', v)} type="email" />
            <Field label="Store / warehouse physical address" value={profile.address} onChange={(v) => set('address', v)} />
          </div>
        </div>

        <div>
          <h3 className="section-title font-bold text-slate-800 text-sm">Google Maps Location &amp; Embed</h3>
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            <Field label="Google Maps Share Link" placeholder="https://maps.app.goo.gl/..." value={config('googleMapLocationUrl') || profile.googleMapLocationUrl || ''} onChange={(v) => { setConfig('googleMapLocationUrl', v); set('googleMapLocationUrl', v); }} />
            <Field label="Google Maps Embed URL" placeholder="https://www.google.com/maps/embed?pb=..." value={config('googleMapEmbedUrl') || profile.googleMapEmbedUrl || ''} onChange={(v) => { setConfig('googleMapEmbedUrl', v); set('googleMapEmbedUrl', v); }} />
            <Field label="GPS Latitude" placeholder="28.7042726" value={config('googleMapLatitude') ?? profile.googleMapLatitude ?? ''} onChange={(v) => { setConfig('googleMapLatitude', v ? Number(v) : ''); set('googleMapLatitude', v ? Number(v) : undefined); }} />
            <Field label="GPS Longitude" placeholder="80.576736" value={config('googleMapLongitude') ?? profile.googleMapLongitude ?? ''} onChange={(v) => { setConfig('googleMapLongitude', v ? Number(v) : ''); set('googleMapLongitude', v ? Number(v) : undefined); }} />
          </div>
        </div>

        <div>
          <h3 className="section-title font-bold text-slate-800 text-sm">Brand assets</h3>
          <div className="mt-3 grid gap-4 md:grid-cols-2">
            <ImageUploadField label="Logo (light mode)" purpose="branding" hint="Shown in the storefront header." value={profile.logoUrl ?? ''} onChange={(v) => set('logoUrl', v)} />
            <ImageUploadField label="Logo (dark mode)" purpose="branding" value={profile.darkLogoUrl ?? ''} onChange={(v) => set('darkLogoUrl', v)} />
            <ImageUploadField label="Favicon" purpose="branding" hint="PNG or ICO, 32x32 or larger." value={profile.faviconUrl ?? ''} onChange={(v) => set('faviconUrl', v)} />
            <ImageUploadField label="Invoice header logo" purpose="branding" value={profile.invoiceLogoUrl ?? ''} onChange={(v) => set('invoiceLogoUrl', v)} />
          </div>
        </div>
      </section>}
